...
describe("SEQUENCE parsing: optional skip, repeated consumption, extra child errors", () => {
  ...
  it("skips trailing optional field at end-of-content", () => {
    const sch = PSchema.constructed("seqTail", { tagNumber: 16 }, [
      PSchema.primitive("id", { tagNumber: 0x02 }, (ab: ArrayBuffer) =>
        new DataView(ab).getUint8(0),
      ),
      PSchema.primitive(
  ...
  it("parses constructed child in sequence", () => {
    const child = PSchema.constructed("box", { tagNumber: 16 }, [
      PSchema.primitive("n", { tagNumber: 0x02 }, (ab: ArrayBuffer) =>
        new DataView(ab).getUint8(0),
      ),
    ]);
    const sch = PSchema.constructed("seqC", { tagNumber: 16 }, [child]);
  ...
  it("throws when required constructed field does not match", () => {
    ...
    const sch = PSchema.constructed("seqCReq", { tagNumber: 16 }, [child]);
...
describe("Depth guard: maxDepth validation", () => {
  it("throws when exceeding maxDepth at nested constructed child", () => {
    const child = PSchema.constructed("box", { tagNumber: 16 }, [
      PSchema.primitive("n", { tagNumber: 0x02 }, (ab: ArrayBuffer) =>
        new DataView(ab).getUint8(0),
      ),
    ]);
    const sch = PSchema.constructed("outer", { tagNumber: 16 }, [child]);
    // outer SEQUENCE containing inner SEQUENCE(INTEGER)
    const buf = fromHexString("30053003020107");
...
describe("Default decode behavior", () => {
  it("primitive without decode function returns raw ArrayBuffer", () => {
    ...
    assert(
      ...
      "Expected ArrayBuffer for default decode",
    );
